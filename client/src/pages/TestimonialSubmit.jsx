  ...
  >
    <div className="flex items-center gap-3 mb-4">
      <div className="h-12 w-12 rounded-full border border-primary/40 overflow-hidden bg-primary/10 flex items-center justify-center text-lg font-semibold text-primary">
        {data.image ? (
          <img src={data.image} alt={data.name || "Profile"} className="w-full h-full object-cover" />
        ) : (
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          {data.role || "Role / Title"} {data.relationship ? `• ${data.relationship}` : ""}
        </p>
      </div>
    </div>
    ...
    <div className="flex gap-1 mb-3">
      {[...Array(5)].map((_, i) => (
        <Star
          key={i}
          size={16}
          className={i < (data.rating ?? 0) ? "text-yellow-400 fill-yellow-400" : "text-muted-foreground/30"}
        />
      ))}
    </div>

    <p className="text-sm text-muted-foreground leading-relaxed italic">
      {data.content || "“Your testimonial will appear here. Keep it concise and specific.”"}
    </p>
  </motion.div>
);
...
export const TestimonialSubmit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatus({ type: "success", msg: "Thank you! Your testimonial is now live on the portfolio." });
      setSubmitted(true);
      setFormData({ name: "", role: "Client / Stakeholder", relationship: "Client", rating: 0, content: "", image: "", link: "", honeypot: "" });
    } catch (error) {
      setStatus({ type: "error", msg: "Could not submit right now. Please try again." });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="relative min-h-screen bg-background text-foreground overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-16 lg:py-20 space-y-10">
        <div className="text-center space-y-3">
          ...
          <h1 className="text-3xl md:text-4xl font-bold">Add Your Testimonial</h1>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            This page is just for people who worked with me—clients, teammates, managers. Your words publish instantly to the portfolio.
          </p>
        </div>
        ...
        {submitted ? (
          ...
          >
            ...
            <p className="text-muted-foreground max-w-xl mx-auto">
              Appreciate your time—your words are already visible on the portfolio. If you want to add another, you can submit again below.
            </p>
            <button
              onClick={() => {
                setSubmitted(false);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            <div className="space-y-4">
              ...
              <div className="bg-card/70 border border-border rounded-2xl p-4 flex items-start gap-3 text-sm text-muted-foreground backdrop-blur-sm">
                ...
                <div>
                  <p className="font-semibold text-foreground mb-1">Who can submit?</p>
                  <p>Clients, collaborators, teammates, managers, and partners. Please keep it genuine and concise.</p>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center gap-2">
                  {[1, 2, 3, 4, 5].map((value) => (
                    ...
                    >
                      <Star
                        size={22}
                        className={value <= formData.rating ? "text-yellow-400 fill-yellow-400" : "text-muted-foreground"}
                      />
                    </button>
                  ))}
                  <span className="text-sm font-semibold text-muted-foreground ml-2">{formData.rating}/5</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-xs font-semibold text-muted-foreground">LinkedIn / Website (optional)</label>
                <input
                  value={formData.link}
