...
import { getSettings, setSettings, clearSettingsCache } from "../../lib/settingsClient";

const targets = [
	{
		id: "all",
		label: "Sync All",
		description: "Sync content, projects, skills, testimonials, certificates",
		icon: CloudUpload,
	},
	{
		id: "content",
		label: "Content",
		description: "Sync hero, about, and site copy",
		icon: FileText,
	},
	{
		id: "projects",
		label: "Projects",
		description: "Sync projects and highlights",
		icon: LayoutPanelLeft,
	},
	{
		id: "skills",
		label: "Skills",
		description: "Sync skills and categories",
		icon: Grid,
	},
	{
		id: "testimonials",
		label: "Testimonials",
		description: "Sync client testimonials",
		icon: MessageCircle,
	},
	{
		id: "certificates",
	...
	const runSync = async (target) => {
		...
		try {
			const res = await fetch(
				target === "all"
					? "/api/admin?action=sync&target=all"
					: `/api/admin?action=sync&target=${target}`,
				{ method: "POST" },
			);
