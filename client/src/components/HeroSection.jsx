...
export const HeroSection = () => {
  ...
  const handleViewResume = () => {
    ...
    window.open(
      hero.resumeUrl || "/gokul-resume.pdf",
      "_blank",
      "noopener,noreferrer",
    );
  };
  ...
  return (
    ...
    >
      <div className="absolute inset-0 -z-10 overflow-hidden">
        ...
        <motion.div
          className="absolute top-20 left-10 w-56 h-56 sm:w-72 sm:h-72 rounded-full bg-gradient-to-r from-primary/10 to-purple-600/10 blur-[100px]"
          animate={{ x: [0, 30, 0], y: [0, -30, 0], scale: [1, 1.1, 1] }}
          transition={{ duration: 15, repeat: Infinity }}
        />
        <motion.div
          className="absolute bottom-20 right-10 w-56 h-56 sm:w-72 sm:h-72 rounded-full bg-gradient-to-r from-cyan-400/10 to-emerald-500/10 blur-[100px]"
                ...
                <div className="font-mono text-sm bg-primary/5 rounded-lg border border-primary/10 min-h-[280px] flex">
                  <div className="p-6 w-full">
                    <div className="grid grid-cols-1 gap-1 h-full content-start">
                      {(hero.codeSnippets || defaultHero.codeSnippets).map(
                        (line, index) => (
                          <div
                            key={index}
                            className={`
